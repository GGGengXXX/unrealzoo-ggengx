...
def get_key_action():
    """
    根据键盘状态生成Mixed动作格式的动作
    返回格式: ([angle, velocity], head_action, animation_action)
    """
    action = ([0, 0], 0, 0)
...
def should_stop_recording():
    """检查是否应该停止录制（按Q或ESC键）"""
    return key_state['q'] or key_state['esc']
...
if __name__ == '__main__':
    ...
    parser.add_argument("--action_mode", type=str, default='keyboard',
                        choices=['random', 'forward', 'circle', 'idle', 'track', 'keyboard'],
                        help='动作模式: random(随机), forward(向前), circle(圆形), idle(静止), track(追踪), keyboard(键盘控制第一个agent)，默认keyboard')
    ...
    env = configUE.ConfigUEWrapper(env, offscreen=True, resolution=tuple(args.resolution))
    ...
    # 创建智能体：第一个agent使用键盘控制，其他使用随机策略
    agents_list = []
    for i in range(len(env.action_space)):
        if i == 0 and args.action_mode == 'keyboard':
            # 第一个agent使用键盘控制，不需要创建ActionAgent
            agents_list.append(None)  # 占位符，实际动作从键盘获取
        else:
            # 其他agent使用随机策略
            agents_list.append(RandomAgent(env.action_space[i]))
    
    if args.action_mode == 'keyboard':
        print(f"创建了 {len(agents_list)} 个智能体：")
        print(f"  - 智能体 0: 键盘控制（使用键盘输入）")
        if len(agents_list) > 1:
            print(f"  - 其他智能体: 使用随机策略")
    else:
    ...
    agent_poses = None  # 智能体位置信息
    manual_stop = False  # 手动停止标志
    ...
    try:
        while step < max_steps and not manual_stop:
            # 检查是否手动停止（按Q或ESC）
            ...
            if len(agents_list) != len(env.action_space):
                ...
                for i in range(len(env.action_space)):
                    if i == 0 and args.action_mode == 'keyboard':
                        agents_list.append(None)  # 第一个agent使用键盘控制
                    else:
                        agents_list.append(RandomAgent(env.action_space[i]))
            ...
            # 在键盘模式下，使用cv2显示第一个agent的观察窗口
            if args.action_mode == 'keyboard' and obs is not None and len(obs) > 0:
                try:
                    # 显示第一个agent的观察
            ...
            # 如果环境提前结束，可以选择重置或退出
            if done and step < max_steps and not manual_stop:
